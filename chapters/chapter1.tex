...
\fm{You could cite the Nat Prot paper of Adham here (in press).}
...
% concrete tasks
\fm{I would move this part up and place it right after the first sentence of this section. this is kind of standard analysis of smFISH, then you add on top of that.}
...
\fm{I have a big of a hard time following the logical flow for the presentation of the Pelkmans lab. not quite sure what you would like to say.}
...
\fm{How do those examples support those conclusions?}
...
A first attempt to build a software self-sufficient for an \ac{smFISH} analysis was the Matlab version of FISH-quant~\cite{mueller_fish-quant_2013}.
...
\fm{Is it not a bit strange to talk about your work in this section which is called related work?}
...
As shown in Figure~\ref{fig:bigfish}, \emph{bigfish} includes several independent subpackages for the different steps in a smFISH analysis workflow: preprocessing, detection, segmentation, and analysis.
...
\fm{Maybe use numbered list 1., 2., 3. with indents to make the text easier digestible}
\fm{Use bold text for the package names?}
...
These methods are gathered in \emph{bigfish.stack}. \fm{bold?}
...
Forth, there are two subpackages dedicated to the analysis of \ac{RNA} distribution.
