...
\noindent
Three mechanisms to transport \ac{mRNA} or locally enrich them are identified~\cite{das_intracellular_2021}:
\begin{enumerate}
	\setlength\itemsep{0.1em}
	...
	This mechanism is notably observed in bacteria, where no active transport of \ac{RNA} has been observed.
\end{enumerate}

\noindent
These mechanisms are illustrated in Figure~\ref{fig:rna_transport}.
...
% mRNA localization allows translation of protein in a specific part of the cell, which have multiple advantages to optimize gene expression.

% Local translation to save energy
% One obvious benefit is energy conservation since an mRNA can be translated multiple times. Producing proteins at their site of action just requires
% moving one transcript instead of moving a large amount of protein.

% Rapid control of gene expression
% Local translation gives to the cell the ability to react rapidly to an external stimulus. For example, local regulation of protein synthesis in
% response to environmental signals plays a crucial role in modulating synaptic plasticity in neurons [40].

% Co-localization of functionally associated transcripts
% Co-localization of functionally associated transcripts can increase the efficiency of the protein complex creation. For instance, the Arp2/3 complex
% plays a major role in the regulation of the acting cytoskeleton. It is composed of 7 subunits which were all shown to localize at the protrusion of migrating cells (Fig 1.12, [55]).
% A computational analysis suggests that the rate of interaction between proteins can strongly depend on their site of translation, and translation at
% some location greatly increases the interaction rate of two proteins (Fig 1.13, [55])

% Localized mRNAs serve as signaling captors
% Localized mRNAs can encode for transcription factors, which are expressed in response to environmental events. These transcription factors are then
% transported to the nucleus to activate other genes. This has been observed in the neurons where dormant mRNA encoding for CREB are stored in the axon and can be
% translated in response to nerve growth factors and are retrogradely trafficked to the cell body [27].
